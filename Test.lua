...
function OrionLib:IsRunning()
	...
	if success then
		...
		if OrionLib.DebugMode then
			warn("IsRunning check failed:", result)
		end
		return false
	end
end
...
local function AddDraggingFunctionality(DragPoint, Main)
	...
	local success, error = pcall(function()
		...
		AddConnection(DragBeganConnection)
		AddConnection(DragChangedConnection)
		AddConnection(UserInputChangedConnection)
	end)
	
	if not success and OrionLib.DebugMode then
		warn("Failed to add dragging functionality: " .. tostring(error))
	end
end   
...
local function LoadCfg(Config)
	...
	if not success then
		if OrionLib.DebugMode then
			warn("Orion Library Config Loader - Failed to decode config: ", Data)
		end
		return
	end
...
local function SaveCfg(Name)
	...
	if not success and OrionLib.DebugMode then
		warn("Orion Library - Failed to save config: ", tostring(result))
		return false
	end
...
function OrionLib:MakeNotification(NotificationConfig)
	task.spawn(function()
		...
		local showTween = TweenService:Create(NotificationFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {Position = UDim2.new(0, 0, 0, 0)})
		showTween:Play()
	...
	if OrionLib.SaveCfg then	
		task.spawn(function()
			local success, fileExists = pcall(function()
				return isfile(OrionLib.Folder .. "/" .. game.GameId .. ".txt")
			end)
			
			if success and fileExists then
				local success, fileContent = pcall(function()
					return readfile(OrionLib.Folder .. "/" .. game.GameId .. ".txt")
				end)
				...
				if success and fileContent then
					...
					if OrionLib.DebugMode then
						warn("Orion Library - Failed to read config file: ", tostring(fileContent))
					end
				end
			elseif OrionLib.DebugMode and not success then
				warn("Orion Library - Failed to check if config file exists: ", tostring(fileExists))
			end
		end)		
	end	
end	
